...
bool WiFiManager::startAP()
{
  ...
  if (!ret)
    DEBUG_WM(DEBUG_ERROR, "[ERROR] There was a problem starting the AP");
...
bool WiFiManager::wifiConnectNew(String ssid, String pass)
{
  ...
  if (!ret)
    DEBUG_WM(DEBUG_ERROR, "[ERROR] wifi begin failed");
  return ret;
}
...
bool WiFiManager::wifiConnectDefault()
{
  ...
  if (!ret)
    DEBUG_WM(DEBUG_ERROR, "[ERROR] wifi enableSta failed");
  ret = WiFi.begin();
  if (!ret)
    DEBUG_WM(DEBUG_ERROR, "[ERROR] wifi begin failed");
  return ret;
...
bool WiFiManager::setSTAConfig()
{
  ...
  if (_sta_static_ip)
  {
    ...
    if (!ret)
      DEBUG_WM(DEBUG_ERROR, "[ERROR] wifi config failed");
    else
...
String WiFiManager::getParamOut()
{
  ...
  if (_paramsCount > 0)
  {
    ...
    for (int i = 0; i < _paramsCount; i++)
    {
      ...
      {
        if (tok_I)
          pitem.replace(FPSTR(T_I), (String)FPSTR(S_parampre) + (String)i);
        if (tok_i)
          pitem.replace(FPSTR(T_i), _params[i]->getID());
        if (tok_n)
          pitem.replace(FPSTR(T_n), _params[i]->getID());
        if (tok_p)
          pitem.replace(FPSTR(T_p), FPSTR(T_t));
        if (tok_t)
          pitem.replace(FPSTR(T_t), _params[i]->getPlaceholder());
        snprintf(parLength, 5, "%d", _params[i]->getValueLength());
        if (tok_l)
          pitem.replace(FPSTR(T_l), parLength);
        if (tok_v)
          pitem.replace(FPSTR(T_v), _params[i]->getValue());
        if (tok_c)
          pitem.replace(FPSTR(T_c), _params[i]->getCustomHTML()); // meant for additional attributes, not html
      }
      else
      {
...
bool WiFiManager::WiFiSetCountry()
{
  ...
  if (ret)
    ...
    DEBUG_WM(DEBUG_ERROR, "[ERROR] esp_wifi_set_country failed");
#endif
  return ret;
}
...
{
  DEBUG_WM(DEBUG_VERBOSE, F("<- HTTP Wifi scan"));
  handleRequest();
  String data = "{\"chipId\":\"{i}\", \"networks\":[{d}]}";
  data.replace("{i}", String(WIFI_getChipId(), HEX));
  String force = String(server->arg(F("force")).c_str());

  if (!_numNetworks || force != "")
    WiFi_scanNetworks(force != ""); // scan in case this gets called before any scans
  ...
  else
  {
    ...
    String items = String("");
    String item_template = "{\"ssid\":\"{s}\",\"quality\":\{q},\"type\":\"{t}\", \"status\":\"{z}\"},";
    //display networks in page
    for (int i = 0; i < n; i++)
    {
      if (indices[i] == -1)
      ...
      if (_minimumQuality == -1 || _minimumQuality < rssiperc)
      {
        ...
        item.replace("{s}", (String)WiFi.SSID(indices[i])); // ssid no encoding
        item.replace("{t}", encryptionTypeStr(enc_type));
        item.replace("{q}", (String)rssiperc); // rssi percentage 0-100
        if (enc_type != WM_WIFIOPEN)
        {
          item.replace("{z}", F("l"));
        }
...
void WiFiManager::handleSaveWifi()
{
  ...
  DEBUG_WM(DEBUG_DEV, F("Sent wifi save"));
  connect = true; //signal ready to connect/reset process in processConfigPortal
}

void WiFiManager::handleCancelWifi()
{
